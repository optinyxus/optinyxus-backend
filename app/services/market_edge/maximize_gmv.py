"""
MAXIMIZE GMV OPTIMIZATION
⚠️ EXACT LOGIC FROM maximize_gmv_marketing.py
"""
...
def run_maximize_gmv_optimization(request_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    MAXIMIZE TOTAL GMV
    ...
    for x1 in x1_vals:
        for x2 in x2_vals:
            for x3 in x3_vals:
                ...
                # Use HARDCODED roas_spread_percent
                result = calculate_gmv_roas(
                    x1, x2, x3, channels, 
                    ROAS_SPREAD_PERCENT  # ✅ Use hardcoded value
                )
    ...
    return {
        "status": "success",
        "message": "Optimal solution found - Total GMV maximized",
        "objective": "maximize_gmv",
        ...
        "seo": {
            ...
            "test_range_min": A["test_min"] if 0 == 0 else 0, # Optimization safe
            "test_range_max": A["test_max"] if 0 == 0 else 0
        },
        "smm": {
            "spend": best["x2"],
            "gmv": r["gmv2"],
